...
playback timings (ms):
  ...
  esindex: 0.017
  cdx.remote: 16.025
  LoadShardBlock: 244.763 (3)
  PetaboxLoader3.datanode: 192.731 (4)
  PetaboxLoader3.resolve: 106.275 (2)
  load_resource: 215.851
*/</div></div></div></av.outerheight()*2;o[at].toolbarmouseover=false;if(av.filter('[data-settings~="mouseover"]').length!=0&&!aa){o[at].toolbarmouseover=true;o[at].toolbarvisible=false;av.css("margin-top","-"+av.outerheight()+"px");av.hide();if(av.filter('[data-settings~="overlay"]').length!=0&&!o[at].hasoneline){av.css("position","absolute");av.css("z-index",2);if(av.filter('[data-settings~="hide"]').length!=0){ar.click(function(){u(at,undefined,undefined,0)});aq.click(function(){u(at,false,undefined,0)})}}else{av.css("z-index",4)}if(av.filter('[data-settings~="delay"]').length!=0){o[at].toolbar_delay=500}ae.mouseenter(function(){u(at,true)}).mouseleave(function(){u(at,false)})}else{if(aa){av.show()}}if(ae.filter('[data-settings~="minimize"]').length==0){j.minimize(at)}if(aq.length!=0&&!aa){if(aq.filter('[data-settings~="dblclick"]').length!=0){ar.dblclick(function(){crayonsyntax.toggleplain(at)})}else{if(aq.filter('[data-settings~="click"]').length!=0){ar.click(function(){crayonsyntax.toggleplain(at)})}else{if(aq.filter('[data-settings~="mouseover"]').length!=0){ae.mouseenter(function(){crayonsyntax.toggleplain(at,true)}).mouseleave(function(){crayonsyntax.toggleplain(at,false)});aa.hide()}}}if(aq.filter('[data-settings~="show-plain-default"]').length!=0){crayonsyntax.toggleplain(at,true)}}var></f.normalizehue[1]){j.x*=f.normalizehuexmulti;j.y*=f.normalizehueymulti}var>